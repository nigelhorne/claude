...
- Filesystem Hostility: Pass empty (0-byte) files, files containing only a single newline (`\n` or `\r\n`), unreadable/unwritable files, directories posing as files, and dangling symlinks. Explicitly test filenames containing spaces, leading/trailing whitespace, and hostile shell metacharacters (e.g., `|`, `>`, `;`) to assert they are parsed safely, do not trigger command injection breaches, and do not cause fatal crashes.
